      ...
      <q-card-section>
        <div class="flex items-center justify-between">
          ...
          <div class="q-px-sm rounded-borders text-white"
            :class="config && config.is_telegram_ready ? 'bg-teal' : 'bg-grey-6'">{{ config && config.is_telegram_ready ?
              'Active' : 'Disabled' }}</div>
        </div>
        <div class="text-caption text-grey-7">Notifikasi order untuk admin via telegram</div>
        <div class="text-caption text-grey-7">Silahkan buat bot di telegram untuk mendapatkan token, serta dapatkan user
          id di bot @infouserid</div>
        <form @submit.prevent="updateData">
          <div class="q-gutter-y-sm q-mt-md">
            <q-input filled v-model="form.telegram_bot_token" label="Telegram Bot Token" />
            <q-input filled v-model="form.telegram_user_id" label="Telegram user Id" placeholder="eg: 1486912253" />
          </div>
          <div class="flex justify-end q-mt-md q-gutter-sm">
            <q-btn :disable="!config.is_telegram_ready" :loading="isLoading" outline size="12px" type="button"
              label="Test Telegram" color="primary" @click="sendTelegram"></q-btn>
        ...
        <form @submit.prevent="updateMailConfig">
          <div class="q-gutter-y-sm q-mt-md">
            ...
            <q-input filled v-model="formEmail.mail_admin" label="Admin Email" placeholder="Email Admin"
              hint="Email admin, semua notifikasi admin akan dikirim ke alamat email diatas" type="email"
              :error="isErrorEmail">
